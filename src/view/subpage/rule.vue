    ...
    <div class="input-wrap">
      <div class="left_rule">
        ...
        <div class="form">
          ...
          <mu-text-field
            v-model="automatic.ddtoken"
            label-float
            full-width
            label="钉钉群推送token(不填默认为云南)"
          ></mu-text-field>
...
export default {
  ...
  methods: {
    automaticTest() {
      let { site, MainSelector, charset, remarks,area, ddtoken } = this.automatic;
     ...
     this.$message({
      ...
      this.$http
        .post("/spider/verification/spiderrule", {
          site,
          MainSelector,
          charset,
          remarks,ddtoken
        ...
        .then(res => {
          if (res.code == 200) {
            ...
            this.$message({
              message:
                "字符集编码需选择utf或gbk,默认为utf，如果验证出现乱码需切换另一个",
              type: "info"
            });
          }
        });
    ...
    ManualTest() {
      ...
      this.$http
        .post("/spider/verification/custom/spiderrule", {
          site,
          MainSelector,
          TitleSelector,
          TimeSelector,
    ...
    ManualSubmit() {
      ...
      confirm("确认爬虫规则获取到的数据验证无误？", "提示", {
        ...
        if (result) {
          this.$http
            .post("/spider/submit/custom/spiderrule", {
              site,
              MainSelector,
              TitleSelector,
              TimeSelector,
